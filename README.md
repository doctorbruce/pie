...
- Windows 使用 `调用工具 powershell {"command":"Start-Sleep -Seconds 10"}` 验证取消（该命令工具由宿主提供）。
...
Core 切换使用统一会话转储：源 Core 导出 canonical transcript，目标 Core 导入并校验后，Astron 才原子更新活动绑定；切回时执行相同流程。两个 Core 保留各自数据库，Astron 会话 ID 不变，原生会话 ID 保存在各自 `coreBindings` 中。Astron 会在首次使用 Pie 时懒启动 sidecar，启动前同步工具和模型提供商，停止、重启和异常拉起也由 adapter 管理；Assistant 的 Skill/Subagent runtime 已按产品配置投影。当前还缺会话工作区隔离和完整产品权限策略。Pie 注册了上游 `read/edit/write/grep/find/ls` 六个工具，以及挂载 Plugin 后的 `load_skill`；命令工具由宿主通过 `PI_TOOLS_DIR` 提供。默认助手暂不启用任何工具。

页面顶部「插件」可导入 Astron `schemaVersion: 1` 的本地插件目录。在「编辑助手」挂载插件，按需启用系统工具，然后新建真实模型会话使用。可先导入仓库内的 `examples/plugins/hello-skill` 验证加载和脚本执行。Plugin 仅负责组织 Skill，APA/App 命令由宿主提供的命令工具执行；依赖环境需要宿主准备。详见 [Plugin 与 Skill](docs/agent-development.md#plugin-与-skill)。

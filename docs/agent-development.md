...
成功加载只证明这次读取了哪条绑定的 SKILL.md，不能证明模型遵循了技能或执行了 APA。后续 `read`、`bash` 独立执行，不继承最近加载的 Skill 来源。APA 执行溯源后续单独设计。

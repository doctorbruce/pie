...
const ProviderConfigSchema = Type.Object({
	name: Type.Optional(Type.String({ minLength: 1 })),
	baseUrl: Type.Optional(Type.String({ minLength: 1 })),
	auth: Type.Optional(Type.Literal("none")),
...
export function parseModelConfig(value: unknown): ModelsJson {
	if (!validateModelsConfig.Check(value)) {
		const paths = validateModelsConfig.Errors(value).map(formatValidationPath).join(", ");
		throw new Error(`Invalid models.json fields: ${paths}`);
	}
	return deepFreeze(structuredClone(value as ModelsJson));
}
